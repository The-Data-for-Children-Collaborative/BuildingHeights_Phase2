...
def resolve_bhm_all(
    datapath_in, datapath_out, maxar_csv, bhm_csv, overwrite_files=False, sentinel=False
):
    ...
    for i in range(len(bhm_df)):
        ...
        if not sentinel:
            ...
            search_string = filecode[:13]
            subcode = filecode[9:13]
            filename = datapath_in + subcode + "-BHM/BHM-" + filecode + ".tif"
        ...
        filename_out = datapath_out + subcode + "-BHM/BHM-" + filecode + "_resolve.tif"
        if isfile(filename_out) and not overwrite_files:
            continue
        if lrx - ulx <= 0:
            print("Skipping, negative x window")
...
def resolve_vhm_all(
    datapath_in, datapath_out, maxar_csv, vhm_csv, overwrite_files=False, sentinel=False
):
    ...
    for i in range(len(vhm_df)):
        ...
        try:
            ...
            continue
        coords = [ulx, uly, lrx, lry]
        filename_out = datapath_out + subcode + "-VHM/VHM-" + filecode + "_resolve.tif"
        if isfile(filename_out) and not overwrite_files:
            continue
        if lrx - ulx <= 0:
            print("Skipping, negative x window")
...
def gdal_georef_sentinel(input_filename, output_filename, coords_in):
    """Resolve BHM file to speficic pixellation.
...
if __name__ == "__main__":
    ...
    if use_subset:
        BHM_init_path = datadir + "height_model_subset/"
    ...
    if use_subset:
        maxar_extent_file = datadir + "grids_extent.csv"
        datapath_in = datadir + "kaggle_maxar_tiles_subset/"
        datapath_out = datadir + "kaggle_maxar_tiles_subset/"
    else:
        maxar_extent_file = datadir + "/grids_extent.csv"
        datapath_in = datadir + "kaggle_maxar_tiles/"
        datapath_out = datadir + "kaggle_maxar_tiles/"
